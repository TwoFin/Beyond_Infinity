...
class controlClass {
    async lowerClass(vmr, level) {
        ...
        console.log("CLIENT_API: New participant security level is: ", level, "=", new_level);
        ...
        if (new_level < current_level)
        {
            ...
            console.log("CLIENT_API: Requesting level change to", json.level, "from:", url)
            var response = await fetch(url, {
                method: 'post',
                body: JSON.stringify(json),
        ...
        {
            console.log("CLIENT_API: No level chenge required as new participant has required clearance")
